...
> 本仓库 ↔ 远端 `pixel4039/study-notes`(默认分支 **master**,不是 main)。这是学习资料的唯一可编辑真源仓库;`resources/study/` 为迁移前旧副本,正式合并/删除需单独计划与用户批准。
...
6. 根目录现存课程目录(`photogrammetry-final-review/`、`spatial_database/`、`spatial_statics/`)是 **legacy bundle**:原样保留,lint 豁免,不得擅自改造、重命名或与 `resources/study/` 互相覆盖。
7. 每个提交单一主题;推送前确认分支为 master;不夹带无关文件。
8. 密钥、令牌、可恢复备份不进本仓库。

## 与其他系统的边界

- Scope Recall 只保存指向本仓库的检索锚点(路径+block ID+origin_date+状态),不存第二份可编辑正文——合同见 `resources/decisions/2026-07-15__proposal__scope-recall-anchor-bridge-contract.md`。
- 运维记录归 hermes-vps-notes(未来 ops-notes),不进本仓库;通用资源归 resources。
